...
class CodegenTools:
        ...
        return [
            ...
            Tool(
                ...
                description=(
                    ...
                    "framework='testfly' (default) emits a TestFly test (extends BaseTest, "
                    "framework-managed driver, open(), accessibility-first getByRole/getByLabel/"
                    "getByTestId/find locators and web-first assertThat assertions — no driver "
                    "lifecycle boilerplate). framework='testng' emits standalone raw Selenium with "
                    "a ChromeDriver setUp/tearDown."
                ),
                inputSchema={
                    "type": "object",
                    "properties": {
                        ...
                        "framework": {
                            "type": "string",
                            "enum": ["testfly", "testng", "selenium_boot"],
                            "default": "testfly",
                            "description": "Output flavor. Use 'testfly' (default) inside a TestFly project."
                        }
                    },
                },
            ),
            Tool(
                ...
                description=(
                    ...
                    "framework='testfly' (default) emits a TestFly test (extends BaseJUnit5Test, "
                    "framework-managed driver, open(), accessibility-first locators and web-first "
                    "assertThat assertions). framework='junit5' emits standalone raw Selenium with "
                    "a ChromeDriver setUp/tearDown."
                ),
                inputSchema={
                    "type": "object",
                    "properties": {
                        ...
                        "framework": {
                            "type": "string",
                            "enum": ["testfly", "junit5", "selenium_boot"],
                            "default": "testfly",
                            "description": "Output flavor. Use 'testfly' (default) inside a TestFly project."
                        }
                    },
            ...
            Tool(
                name="detect_testfly",
                description=(
                    "Detect whether the current working directory is inside a TestFly "
                    "project (looks for testfly.yml or the io.testfly "
                    ...
                    "generate with framework=\"testfly\" so the output uses the "
                    "framework's accessibility-first locators, open(), and managed driver."
                ),
                inputSchema={
                    "type": "object",
                    "properties": {
            ...
            Tool(
                ...
                description=(
                    "Backward-compatible alias for detect_testfly. Detects whether the current working "
                    "directory is inside a TestFly project."
                ),
                inputSchema={
                    "type": "object",
                    "properties": {
            ...
            Tool(
                name="generate_testfly_config",
                description=(
                    "Generate a production-ready testfly.yml configuration file tailored to the current "
                    "session or customized via arguments."
                ),
                inputSchema={
                    "type": "object",
                    "properties": {
                        "base_url": {
                            "type": "string",
                            "description": "Base URL of the application. Auto-inferred from session if omitted."
                        },
                        "browser": {
                            "type": "string",
                            "enum": ["chrome", "firefox"],
                        ...
                        "parallel": {
                            "type": "string",
                            "enum": ["none", "methods", "classes"],
                            "default": "methods",
                            "description": "TestNG parallel execution mode"
                        },
                        "thread_count": {
                            "type": "integer",
                            "default": 4,
                        ...
                        "retry_attempts": {
                            "type": "integer",
                            "default": 2,
                            "description": "Number of retry attempts for flaky tests (0 to disable)"
                        },
                    },
                },
            ),
            Tool(
                name="generate_testfly_pom",
                description=(
                    "Generate a standard Maven pom.xml pre-configured with TestFly 1.0.0, "
                    "Java 17+, maven-compiler-plugin, and maven-surefire-plugin."
                ),
                inputSchema={
                    "type": "object",
                    "properties": {
                        ...
                        "testfly_version": {
                            "type": "string",
                            "default": "1.0.0",
                            "description": "TestFly dependency version"
                        },
                    },
                },
            ),
            ...
            Tool(
                ...
                description=(
                    ...
                    "TestNG Cucumber runner from the recorded browser session. "
                    "framework='testfly' (default) emits steps extending BaseCucumberSteps with "
                    "framework-managed driver, open(), accessibility-first locators, web-first assertThat, "
                    "and a RunCucumberTest runner extending BaseCucumberTest. framework='raw' emits standalone Selenium."
                ),
                inputSchema={
                    "type": "object",
                    "properties": {
                        ...
                        "framework": {
                            "type": "string",
                            "enum": ["testfly", "raw", "selenium_boot"],
                            "default": "testfly",
                            "description": "Step-definition flavor. Use 'testfly' (default) inside a TestFly project."
                        }
                    },
                },
            ),
            ...
            Tool(
                ...
                description=(
                    ...
                    "Test class that uses it. Supports TestFly (framework='testfly', default): Page Object extends "
                    "BasePage, Test extends BaseTest, framework manages driver lifecycle, accessibility-first locators "
                    "and web-first assertThat assertions. framework='testng'/'junit5' emit standalone raw Selenium."
                ),
                inputSchema={
                    "type": "object",
                    "properties": {
                        ...
                        "framework": {
                            "type": "string",
                            "enum": ["testfly", "testng", "junit5", "selenium_boot"],
                            "description": (
                                "Output flavor: 'testfly' (default) emits TestFly (BasePage/BaseTest, "
                                "framework-managed driver, web-first assertions); 'testng'/'junit5' emit "
                                "standalone Selenium with a ChromeDriver setUp/tearDown."
                            ),
                            "default": "testfly"
                        }
                    },
    ...
    def get_handlers(self) -> dict:
        return {
            ...
            "detect_testfly":             self._detect_testfly,
            "detect_selenium_boot":       self._detect_testfly,
            "generate_testfly_config":    self._generate_testfly_config,
            "generate_testfly_pom":       self._generate_testfly_pom,
    ...
    async def _detect_testfly(self, args: dict) -> str:
        result = detect_testfly(args.get("project_dir"))
        if result["detected"]:
            ev = "\n".join(f"  - {e}" for e in result["evidence"])
            return (
                "✅ TestFly project DETECTED"
                f" (root: {result['root']}).\n{ev}\n\n"
                "Generate Java with framework=\"testfly\" so the code uses the "
        ...
        return (
            "No TestFly markers found near the working directory. "
            "If this IS a TestFly project, pass project_dir or generate with "
            "framework=\"testfly\" explicitly; otherwise use the plain "
            "testng / junit5 flavors."
        )

    # ------------------------------------------------------------------ #
    #  TestFly configuration & pom codegen                                 #
    # ------------------------------------------------------------------ #
    async def _generate_testfly_config(self, args: dict) -> str:
        log = self.browser._session_log
        base_url = args.get("base_url")
        if not base_url:
            base_url = next((e["url"] for e in log if e.get("action") == "navigate"), "https://example.com")
            parsed = urlparse(base_url)
            if parsed.scheme and parsed.netloc:
                base_url = f"{parsed.scheme}://{parsed.netloc}"

        browser = args.get("browser", "chrome")
        headless = "true" if args.get("headless", False) else "false"
        parallel = args.get("parallel", "methods")
        thread_count = args.get("thread_count", 4)
        explicit_timeout = args.get("explicit_timeout", 10)
        page_load_timeout = args.get("page_load_timeout", 30)
        retry_attempts = args.get("retry_attempts", 2)
        retry_enabled = "true" if retry_attempts > 0 else "false"

        config_yaml = f"""execution:
  mode: local
  baseUrl: {base_url}
  parallel: {parallel}
  threadCount: {thread_count}
  maxActiveSessions: {thread_count}
...
"""
    ...
    async def _generate_testfly_pom(self, args: dict) -> str:
        group_id = args.get("group_id", "com.example")
        artifact_id = args.get("artifact_id", "testfly-automation")
        version = args.get("version", "1.0.0-SNAPSHOT")
        testfly_version = args.get("testfly_version", "1.0.0")
...
'''
    ...
    async def _generate_java_testng(self, args: dict) -> str:
        ...
        if args.get("framework", "testfly") in ("testfly", "selenium_boot"):
            return self._java_sb_inline_test(
                test_name, package, log, base_class="BaseTest",
                base_class_import="import io.testfly.test.BaseTest;",
...
'''
    ...
    async def _generate_java_junit5(self, args: dict) -> str:
        ...
        if args.get("framework", "testfly") in ("testfly", "selenium_boot"):
            # BaseJUnit5Test exposes find()/assertThat()/open() and getBy* factories
            return self._java_sb_inline_test(
                test_name, package, log, base_class="BaseJUnit5Test",
                base_class_import="import io.testfly.junit5.BaseJUnit5Test;",
                test_annotation="@Test",
                test_annotation_import="import org.junit.jupiter.api.Test;",
                static_ctx=False, assert_api="junit5")
...
'''
    ...
    async def _generate_java_page_object(self, args: dict) -> str:
        ...
        framework = args.get("framework", "testfly")
        ...
        if framework in ("testfly", "selenium_boot"):
    ...
    # ------------------------------------------------------------------ #
    #  TestFly flavor (BasePage / BaseTest)                                #
    # ------------------------------------------------------------------ #
    _ROLE_ENUM = {
        ...
        "list": "LIST", "listitem": "LISTITEM", "table": "TABLE", "row": "ROW",
        "cell": "CELL", "columnheader": "COLUMNHEADER", "navigation": "NAVIGATION",
        "banner": "BANNER", "contentinfo": "CONTENTINFO", "main": "MAIN",
        "region": "REGION", "article": "ARTICLE", "dialog": "DIALOG", "alert": "ALERT",
        "alertdialog": "ALERTDIALOG", "tab": "TAB", "tablist": "TABLIST",
        "tabpanel": "TABPANEL", "menu": "MENU", "menuitem": "MENUITEM",
        "progressbar": "PROGRESSBAR", "spinbutton": "SPINBUTTON", "slider": "SLIDER",
    ...
    def _by_wrap(self, by_expr: str, static_ctx: bool) -> str:
        """find(By.x(...)) in instance context; Locator.of(By.x(...)) in static context."""
        return f"Locator.of({by_expr})" if static_ctx else f"find({by_expr})"

    def _css_wrap(self, css: str, static_ctx: bool) -> str:
        q = lambda s: s.replace("\\", "\\\\").replace('"', '\\"')
        return f'Locator.ofCss("{q(css)}")' if static_ctx else f'find("{q(css)}")'
    ...
    def _java_sb_page_class(self, page_name: str, package: str, elements: dict) -> str:
        """Page Object extending io.testfly.test.BasePage.
        ...
        if "Role." in rendered:
            imports.append("import io.testfly.locator.Role;")
        if "By." in rendered:
    ...
    def _sb_assertion_lines(self, entry: dict, static_ctx: bool = False,
        ...
        if action == "assert_title":
            ...
            if entry.get("exact"):
                return [f'assertThat(getDriver()).hasTitle("{exp}");']
            return [f'assertThat(getDriver()).titleContains("{exp}");']
        if action == "assert_url":
            exp = q(entry.get("expected", ""))
            if entry.get("exact"):
                return [f'assertThat(getDriver()).hasUrl("{exp}");']
            return [f'assertThat(getDriver()).urlContains("{exp}");']
    ...
    def _sb_inline_imports(self, rendered: str, base_class_import: str,
        ...
        if static_ctx and ("Locator." in rendered):
            imports.append("import io.testfly.locator.Locator;")
        if "Role." in rendered:
            imports.append("import io.testfly.locator.Role;")
        if "By." in rendered:
            imports.append("import org.openqa.selenium.By;")
    ...
    def _java_sb_test(self, test_name, package, pages_package, page_name,
        ...
        """Test extending io.testfly.test.BaseTest.
        ...
        execution.baseUrl in testfly.yml; a cross-origin hop falls back to
        ...
        body_lines = []
        if base_url:
            body_lines.append(f"{i}{i}// baseUrl (execution.baseUrl in testfly.yml) should be the site origin")
    ...
    async def _generate_gherkin(self, args: dict) -> str:
        ...
        framework = args.get("framework", "testfly")
        if framework in ("testfly", "selenium_boot"):
            runner_class = "RunCucumberTest"
            runner = self._gherkin_runner_sb(runner_class, package, steps_package)
    ...
    def _gherkin_runner_sb(self, class_name: str, package: str, steps_package: str) -> str:
...
public class {class_name} extends BaseCucumberTest {{
}}
'''
