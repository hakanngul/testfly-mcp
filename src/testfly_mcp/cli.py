...
def cmd_interactive():
    ...
    while True:
        print("\n========================================================")
        print(f"✈  TestFly MCP Server (v{ver}) — Interactive CLI")
        print("========================================================")
        print("  [1] Launch Interactive Web Studio (http://127.0.0.1:8765)")
        print("  [2] Run Environment Doctor (Diagnostics)")
        print("  [3] List MCP Tools (88 tools)")
        print("  [4] Initialize testfly.yml in Current Directory")
        print("  [5] Start MCP Stdio Server (for AI Assistants)")
        print("  [q] Exit")
        print("--------------------------------------------------------")
        ...
        if choice == "1":
            ...
            print("Invalid selection. Please enter a number 1 to 5 or 'q'.")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="testfly-mcp",
        description=f"TestFly MCP Server (v{ver}) — Browser Automation & TestFly Framework Codegen",
        epilog="Examples:\n"
               "  testfly-mcp                     # Auto-detect: launch interactive menu (TTY) or stdio server (IDE/pipe)\n"
               "  testfly-mcp ui                  # Launch interactive web studio in browser\n"
               "  testfly-mcp doctor              # Verify Python, Selenium, Chrome, and IDE setup\n"
               "  testfly-mcp tools               # List all 88 available MCP tools\n"
               "  testfly-mcp init-config         # Generate standard testfly.yml in current directory\n"
               "  testfly-mcp stdio               # Explicitly run MCP stdio server\n",
    ...
    # init-config
    cfg_p = subparsers.add_parser("init-config", help="Generate standard testfly.yml template")
